...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        /*
         $source =  preg_replace_callback("/<\?[^><]+\?>|<\%[^><]+\%>|<script[^>]+language[^>]*=[^>]*php[^>]*>[^><]*<\/script\s*>/iU", "", $source);
         return  preg_replace_callback("/{([^\}\{\n]*)}/e", "\$this->select('\\1');", $source);
         */
        
        $source = preg_replace_callback ( "/<\?[^><]+\?>|<\%[^><]+\%>|<script[^>]+language[^>]*=[^>]*php[^>]*>[^><]*<\/script\s*>/iU",
        		function($m){
        			return "";
        		}
        , $source );
...
//             $source      = preg_replace($pattern, $replacement, $source);
        	  ...
        	  // $replacement = "'{include file='.strtolower('\\1'). '}'";
        	   $source      = preg_replace_callback($pattern, function($m){
        	   	return '{include file='.strtolower($m[1]). '}';
        	   }, $source);
